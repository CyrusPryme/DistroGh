-- Balance: deduct returns at recorded unit_price (fallback catalog vendor_price).
-- Keeps reporting.vendor_balances aligned with lib/vendor-earnings.ts.
...
COMMENT ON VIEW reporting.vendor_balances IS
  'Vendor balance = settled sales vendor_due − returns (at return unit_price) − deductions − payouts recorded (non-failed).';
