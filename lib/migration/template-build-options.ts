...
import { fetchActiveProductNames, fetchActiveProductBarcodes } from '@/lib/migration/template-products'
import {
...
export async function fetchMigrationTemplateBuildOptions(db: Pool): Promise<TemplateBuildOptions> {
  const [vendorNames, productNames, productBarcodes, supermarketBranchLabels, supermarketNames, categoryNames] =
    await Promise.all([
      fetchActiveVendorNames(db),
      fetchActiveProductNames(db),
      fetchActiveProductBarcodes(db),
