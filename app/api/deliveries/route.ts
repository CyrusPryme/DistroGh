...
export async function GET(req: Request) {
  try {
    ...
    const vendorFilter =
      session.role === 'vendor' ? (session.vendor_id ?? null) : vendor_id_param || null
    if (session.role === 'vendor' && !vendorFilter) {
      return NextResponse.json({ success: false, error: 'Forbidden' }, { status: 403 })
    }

    const requireConfirmed = session.role === 'vendor' || confirmed_only
    ...
    const { rows } = await pool.query(
      ...
      where dr.deleted_at is null
        ...
        and ($5::boolean is false or dr.confirmed_at is not null)
      order by dr.delivery_date desc
      limit 20000
      `,
      [vendorFilter, supermarket_id, from, to, requireConfirmed]
    )
